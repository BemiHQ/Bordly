...
const MenuBar = ({ editor }: { editor: ReturnType<typeof useEditor> }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 pt-3 px-3">
      {/* Font Family */}
      <Select
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            className={cn('h-7 px-2', editor.isActive('blockquote') && 'bg-muted')}
          >
      ...
      >
        <Tooltip>
          <TooltipTrigger asChild>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={cn('h-7 px-2', editor.isActive('link') && 'bg-muted')}>
...
export const ReplyCard = ({ onCancel }: { boardId: string; boardCardId: string; onCancel: () => void }) => {
